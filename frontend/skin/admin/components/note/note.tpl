...
<div class="{$component} {cmods name=$component mods=$mods} {$classes}" data-param-user_id="{$targetId}" {cattr list=$attributes}>
    ...
    {if $isEditable}
        <form class="{$component}-form js-note-form" style="display: none;">
            {component 'admin:field' template='textarea' inputClasses="$component-form-text js-note-form-text"}

            {component 'admin:button' mods='primary' text=$aLang.common.save}
            {component 'admin:button' type='button' classes="js-note-form-cancel" text=$aLang.common.cancel}
        </form>
    {/if}
</div>